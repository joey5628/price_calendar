...
* updated by hxke, 2011-04-08
*/
jQuery(function () {
    jQuery.fn.calendar = function (options) {
        var calendar = jQuery.fn.calendar;
        calendar.init();
    ...
    jQuery.fn.calendar.init = function () {
        var self = jQuery.fn.calendar;
    ...
    jQuery.fn.calendar.Frame = function () {
        this.container = jQuery(['<div id="calendar" class="gs-date" style="display:none">',
			'<div id="datelist0" class="date-list"></div>',
			'<div id="datelist1" class="date-list"></div>',
        ...
        var that = this;
        that.units = jQuery('#datelist0, #datelist1').map(function () {
            var unit = new jQuery.fn.calendar.Unit(this);
    ...
    jQuery.fn.calendar.Frame.prototype = {
    ...
    jQuery.fn.calendar.Unit = function (el, options) {
        var elem = jQuery(el);
    ...
    jQuery.fn.calendar.Unit.prototype = {
        character: {
            y: "\u5e74",
            m: "\u6708",
            weekday: "\u65e5\u4e00\u4e8c\u4e09\u56db\u4e94\u516d"
        ...
        options: {
            ...
            minDatefilter: function (date, mindate, maxdate) {
                return date >= mindate && date < maxdate;
            },

            maxDatefilter: function (date, maxdate) {
                return date > maxdate;
            }
        },
        set: function (year, month) {
            ...
            var mindatef = this.options.minDatefilter;
            var maxdatef = this.options.maxDatefilter;

            //add 


            var isForetime = jQuery.fn.calendar.target.data('calendar.options').foretime;
            ...
            var minDateStr = jQuery.fn.calendar.target.data('calendar.options').minDate;
            var maxDateStr = jQuery.fn.calendar.target.data('calendar.options').maxDate;
            if(minDateStr != '' && maxDateStr != ''){
                var minDate = new Date(minDateStr);
                minDate = new Date(minDate.addDays(-1));
                var maxDate = new Date(maxDateStr); 
                var td = new Date(this.year, this.month, 1);               
                this.numbers.each(function (i) {
                    td.setDate(i + 1);
                    console.log(td + ' : '+!mindatef(td, minDate, maxDate));
                    jQuery(this).toggleClass('foretime', !mindatef(td, minDate, maxDate));
                });
